  ...
  fields: {
    ...
    elo: {
      ...
      async resolve() {
        ...
        const histo = matches.reduce(
          (acc, { homeTeam, awayTeam, winnerUid }) => {
            if (!acc[homeTeam]) {
              acc[homeTeam] = 1000;
            }
            if (!acc[awayTeam]) {
            ...
            const eloForHomeTeam = getEloRanking(
              acc[homeTeam],
              acc[awayTeam],
              homeTeam === winnerUid ? 1 : 0
            );
        ...
        return campuses.map(campuse => ({
          ...campuse,
          elo: histo[campuse.uid]
        }));
      }
    }
  }
